...
with open(BASE_DIR / "call_data.txt", 'r', encoding='utf-8') as file:
    conversation_examples = file.read()
#print (conversation_examples)

#rules of conversations from the side of agent
with open(BASE_DIR / "prompt.txt", 'r', encoding='utf-8') as file:
    rules = file.read()
#print (rules)

with open(BASE_DIR / "call_data_external.txt", 'r', encoding='utf-8') as file:
    conversation_examples_external = file.read()

with open(BASE_DIR / "call_data_internal.txt", 'r', encoding='utf-8') as file:
...
generated_dir = BASE_DIR / "generated_conversations"
generated_dir.mkdir(exist_ok=True)
date = datetime.now().strftime("%Y%m%d_%H%M%S")
with open(generated_dir / ("debtor_agent_" + date + ".txt"), 'w') as file:
    file.write(conversation)

#save the conversation as an example for the similarity check
with open(BASE_DIR / "generated_example.txt", 'w') as file:
    file.truncate(0)
    file.write(conversation)
