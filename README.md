...
source conversation/bin/activate          # macOS / Linux
conversation\Scripts\activate             # Windows
```
...
3. Set up your environment variables:
   - A `.env` file is included in the project root. If it is missing, copy `.env.example` to `.env`.
   ...
   - The scripts read this variable automatically via `python-dotenv`; you no longer need to edit `conversation.py` or `similarities.py`.
